    ...
    public class CH340SerialPort extends CommonUsbSerialPort {
        ...
        private void checkState(String msg, int request, int value, int[] expected)
            ...
            if (ret != expected.length) {
                throw new IOException("Expected " + expected.length + " bytes, but get " + ret
                        + " [" + msg + "]");
            }
            ...
            for (int i = 0; i < expected.length; i++) {
                ...
                if (expected[i] != current) {
                    throw new IOException("Expected 0x" + Integer.toHexString(expected[i])
                            + " bytes, but get 0x" + Integer.toHexString(current) + " [" + msg
                            + "]");
                }
            }
        }
        ...
        private void initialize() throws IOException {
            checkState("init #1", 0x5f, 0, new int[] {
                    -1 /* 0x27, 0x30 */, 0x00
            });
            ...
            checkState("init #4", 0x95, 0x2518, new int[] {
                    -1 /* 0x56, c3 */, 0x00
            });

            // Commented by Upendra as it was not supporting some devices
//            if (controlOut(0x9a, 0x2518, 0x0050) < 0) {
//                throw new IOException("init failed! #5");
//            }
//
//            checkState("init #6", 0x95, 0x0706, new int[] {
//                    0xff, 0xee
//            });
//
...
//            }
            ...
            checkState("init #10", 0x95, 0x0706, new int[] {
                    -1/* 0x9f, 0xff */, 0xee
            });
        }
