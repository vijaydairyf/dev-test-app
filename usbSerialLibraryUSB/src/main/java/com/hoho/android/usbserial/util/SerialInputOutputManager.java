    ...
    public void run() {
       // Process.setThreadPriority(Process.THREAD_PRIORITY_BACKGROUND);
        ...
        try {
            ...
            int srcOffset=0;
            int byteCount=0;
            ByteBuffer readBuffer=ByteBuffer.allocate(BUFSIZ);
            ...
            while (true) {
               ...
               // ++count;
                if(readData!=null)
                {
                    readBuffer.put(readData,0,readData.length);
                }

                if(readBuffer.position()>0)
                {
                    if((++count%8==0)||(readBuffer.position()>200))
                    {
                        int length=readBuffer.position();
                        final byte[] dataToReturn=new byte[length];
                        readBuffer.position(0);
                        readBuffer.get(dataToReturn,0,length);
                        listener.onNewData(dataToReturn);
                        readBuffer.clear();
                    }
                }
//                else if(count % 25 == 0){
...
//                }
    ...
    private byte[] step() throws IOException {
...
//            }
    ...
    /**
     * Created for testing purpose by Upendra
     * @param sFileName
     * @param sBody
     * @param folderName
     * @throws IOException
     */
//    public static void generateNoteOnSD(String sFileName, String sBody, String folderName) throws IOException  {
//
